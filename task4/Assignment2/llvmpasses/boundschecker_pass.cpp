...
static void initMain(Function *FN) {
  ...
  FunctionCallee hook =
      FN->getParent()->getOrInsertFunction("__runtime_main_prologue", hookfty);
...
static bool instrumentMallocCall(CallBase *CB) {
  ...
  Type *ReturnTy = Type::getInt8PtrTy(CB->getModule()->getContext());
  Type *ArgumentTy = Type::getInt64Ty(CB->getModule()->getContext());
  SmallVector<Type *, 1> ArgTypeList;
  ArgTypeList.push_back(ArgumentTy);
  FunctionType *hookfty = FunctionType::get(ReturnTy, ArgTypeList, false);
  FunctionCallee hook =
      CB->getModule()->getOrInsertFunction("__runtime_mymalloc", hookfty);

  CB->setCalledFunction(hook);
  return true;
}
...
static bool instrumentMemoryRead(LoadInst *LI) {
  ...
  FunctionType *hookfty = FunctionType::get(ReturnTy, ArgTypeList, false);
  FunctionCallee hook =
      LI->getModule()->getOrInsertFunction("__runtime_checkbound", hookfty);

  /*  Create a call to the function __runtime_checkbound  */
  IRBuilder<> B(LI);
  Value *Ptr = LI->getPointerOperand();
  /*  Typecast from arbitrary pointer type to runtime function's parameter type
   * (void*). LLVM has strict type policy. */
  ...
  /*  set the pointer operand with a return value of __runtime_checkbound (dummy
   * tho) */
...
static bool instrumentMemoryWrite(StoreInst *SI) {
  ...
  FunctionType *hookfty = FunctionType::get(ReturnTy, ArgTypeList, false);
  FunctionCallee hook =
      SI->getModule()->getOrInsertFunction("__runtime_checkbound", hookfty);
  ...
  virtual bool runOnModule(Module &M) {
    ...
    for (auto F = M.begin(); F != M.end(); ++F) {
      ...
      if (F->getName().equals("main")) {
        /*  This inserts a call to a run-time function, that set up data
         * structures */
      }
      for (auto BI = F->begin(); BI != F->end(); ++BI) {
        for (auto II = BI->begin(); II != BI->end(); ++II) {
          ...
          if (auto *LI = dyn_cast<LoadInst>(Ins)) {
            /* We skip checking memory access to constant (e.g. global
             * variables), since we check only heap for this assignment */
...
static RegisterPass<BoundsCheckerModule> X("boundschecker",
                                           "Bounds Checker Pass", false, false);
...
static RegisterStandardPasses
    RegisterMyPass(PassManagerBuilder::EP_ModuleOptimizerEarly, registerPass);
